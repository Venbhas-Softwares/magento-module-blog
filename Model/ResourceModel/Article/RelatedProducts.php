...
class RelatedProducts implements RelatedProductsResourceInterface
{
    ...
    /**
     * Get article ids that reference a product as related.
     *
     * @param int $productId
     * @return int[]
     */
    public function getRelatedArticleIdsByProductId(int $productId): array
    {
        if ($productId <= 0) {
            return [];
        }
        ...
        return array_values(array_filter(array_map('intval', is_array($result) ? $result : [])));
    }
